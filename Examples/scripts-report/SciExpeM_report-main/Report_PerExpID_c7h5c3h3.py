...
ID_ChemModels = [756, 758, 757, ]
labels = ['oldtherm', 'newtherm', 'newkin']#,'','','','','']
#exps_type =  ['outlet concentration measurement']
list_exps = {'burner stabilized flame speciation measurement': [
       # fiamme C2-4 premixed speciazione
       3849, 3855, 3083, 3850, 3851, 3852, 3087,
       # tol fiamme
       3221, # LI 2011
       3222, # LI 2011
       3217, # LI 2011
    ...
    3134,  # C7H8 #
    3615,  # C7H8 # simul not ended non la carica
    3614,  # C7H8+C3H6 # simul not ended non la carica
    3613,  # C7H8+C3H4-P # simul not ended non la carica
    3610,  # C7H8+C2H4
    3609,  # C7H8+C2H2
    3608,  # C7H8+C2H2
    3607,  # C7H8+C2H2
    3595,  # C3H6   # simul not ended non la carica
    3596,  # C3H4-P   # simul not ended non la carica
    3597,  # CYC5H8
    3599,  # CYC5H8
    3600,  # CYC5H8+C2H2
    3602,  # NC7H16
...
exps_type = ['burner stabilized flame speciation measurement',  ]# 'jet stirred reactor measurement', ]
...
fuels = ['C2H2','C2H4','C3H4-A','C3H4-P','C4H6','C5H6','CYC5H8','C7H8', 'NC7H16']          # fill with list of desired fuels. If empty all fuels are retrieved
...
savepath = r'/Users/lpratalimaffei/Library/CloudStorage/OneDrive-PolitecnicodiMilano/Luna/Universita/RICERCA/ZZZ-PUBBLICAZIONI/2025_C7H5C3H3/SIMULREPORT'
...
color = ['r','k','k','k','b','b','b','b','r','r','r','r','g','g','g','g',]
linstyle = [':','--','-',':','-','--','-.',':','-','--','-.',':','-','--','-.',':']
fs = 8          # fontsize
...
for exp_type in exps_type:
    ...
    preferred_order_species_new = fuels
    ...
    for j in list_exps[exp_type]:
        ...
        for target in targets:
            ...
            elif idjj < 4*Nplots: npag = 4
            elif idjj < 5*Nplots: npag = 5
            print(idjj, Nplots, npag)
            plt.subplot(int(math.sqrt(Nplots)),int(math.sqrt(Nplots)),idjj+1-Nplots*(npag-1))
            if len(y_unc_abs)>0:
                plt.errorbar(x_exp,y_exp, yerr=y_unc_abs, fmt = 'o', markersize = 6, capsize=3, elinewidth=1, capthick=1, c='k', mfc = 'none')
            else:
                plt.scatter(x_exp,y_exp, edgecolor='k', facecolor = 'none') # plt.scatter non funziona (forse) per plottare le errorbars
            ...
            for idI, ID_ChemModel in enumerate(ID_ChemModels):
                ...
                if has_profiles is True:
                    if len(x_exp) == len(x_sim):
                        print(
                            '*Warning: exp with profiles. plotting with x_sim = x_exp until bug will be fixed')
                        plt.plot(
