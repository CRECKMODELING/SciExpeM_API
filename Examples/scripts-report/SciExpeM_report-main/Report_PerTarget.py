...
exps_type =  ['laminar burning velocity measurement']#'jet stirred reactor measurement',
...
fuels = []          # fill with list of desired fuels. If empty all fuels are retrieved
...
savepath =  r'D:\POLI\OPENSMOKE\ALKANES\NC7' # creare in un path a piacere la cartella 'report', 
...
for exp_type in exps_type:
    ...
    for fuel in fuels:
        ...
        pdf = PdfPages(os.path.join(savepath,exp_name,f"{fuel_savename}_{exp_name}.pdf")) 
        savepdf = False
            ...
            for i in range(1,Nsubplots+1):
                ...
                for idj, j in enumerate(exp_split_rows[i-1]):
                    ...
                    for idI, ID_ChemModel in enumerate(ID_ChemModels):
                        ...
                        if warn_sim == True:
                            warn_sim_final = True
                        plt.plot(x_sim,y_sim, linstyle[idI], c=color[idI], label = ID_ChemModel)
